...
void inputTransform5(vector<float>& V, vector<float>input, int input_n, int input_c, int input_h, int input_w) {
	...
	for (int n = 0; n < input_n; n++) // N
	{
		...
		for (int inch = 0; inch < input_c; inch++) // IC
		{
			...
			for (int row = 0; row < output_H; row += 2)  // T_N
			{
				...
				{
					int temp_v3 = tempic2 + tempic3 + (tilecount + col / 2) * 16;
					...
					V[temp_v3 + 4] = *d5 + *d9 - dd3;
					V[temp_v3 + 5] = dd4 + dd3;
					V[temp_v3 + 6] = dd3 - dd4;
					V[temp_v3 + 7] = dd4 - *d8 - *d12;

					
					V[temp_v3 + 8] = *d9 - *d5 + dd5;
					V[temp_v3 + 9] = dd6 - dd5;
					V[temp_v3 + 10] = -(dd6 + dd5);
					V[temp_v3 + 11] = dd6 + *d8 - *d12;

					
					V[temp_v3 + 12] = *d5 - *d13 + dd7;
					V[temp_v3 + 13] = dd8 - dd7;
					V[temp_v3 + 14] = -dd7 - dd8;
					V[temp_v3 + 15] = dd8 - *d8 + *d16;

					/*
					cout << setw(5) << V[temp_v3] << " " << setw(5) << V[temp_v3 + 1] << " " << setw(5) << V[temp_v3 + 2] << " " << setw(5) << V[temp_v3 + 3];
					cout << setw(5) << V[temp_v3 + 4] << " " << setw(5) << V[temp_v3 + 5] << " " << setw(5) << V[temp_v3 + 6] << " " << setw(5) << V[temp_v3 + 7];
					cout << setw(5) << V[temp_v3 + 8] << " " << setw(5) << V[temp_v3 + 9] << " " << setw(5) << V[temp_v3 + 10] << " " << setw(5) << V[temp_v3 + 11];
					cout << setw(5) << V[temp_v3 + 12] << " " << setw(5) << V[temp_v3 + 13] << " " << setw(5) << V[temp_v3 + 14] << " " << setw(5) << V[temp_v3 + 15];
					cout << endl;
					*/
...
void MatrixMulElement4X4(vector<float> matrix_U, vector<float> matrix_V, vector<float>& matrix_M, int U_rows, int UcolsVrows, int V_cols, int tile_h, int tile_w)
{
	int Output_c = U_rows / 4;
	int Input_c = UcolsVrows / 4;
	int Input_n = V_cols / (4 * tile_h * tile_w);
	//int tile_n = tile_n; // tile 총 개수 
	int Output_W = tile_w * 2;
	int Output_H = tile_h * 2;
	...
	for (int oc = 0; oc < Output_c; ++oc) { // OC
		...
		for (int n = 0; n < Input_n; ++n) // N
		{
			...
			for (int row = 0; row < Output_H; row += 2) //TILE
			{

				int row_idxo3 = (row / 2) * (Output_W / 2);
				...
				for (int col = 0; col < Output_W; col += 2)//TILE
				{
					...
					for (int ic = 0; ic < Input_c; ++ic) // IC
					{
						...
						int midx2 = (ic * Input_n * tile_h * tile_w * 16) + (n * tile_h * tile_w * 16) + (col / 2 + row_idxo3) * 16; // 최적화필요
						...
						int midx12 = midx1 + Input_c * 4;
						m5 += matrix_U[midx12] * matrix_V[midx2 + 4];
						m6 += matrix_U[midx12 + 1] * matrix_V[midx2 + 5];
						m7 += matrix_U[midx12 + 2] * matrix_V[midx2 + 6];
						m8 += matrix_U[midx12 + 3] * matrix_V[midx2 + 7];

						int midx13 = midx12 + Input_c * 4;
						m9 += matrix_U[midx13] * matrix_V[midx2 + 8];
						m10 += matrix_U[midx13 + 1] * matrix_V[midx2 + 9];
						m11 += matrix_U[midx13 + 2] * matrix_V[midx2 + 10];
						m12 += matrix_U[midx13 + 3] * matrix_V[midx2 + 11];

						int midx14 = midx13 + Input_c * 4;
						m13 += matrix_U[midx14] * matrix_V[midx2 + 12];
						m14 += matrix_U[midx14 + 1] * matrix_V[midx2 + 13];
						m15 += matrix_U[midx14 + 2] * matrix_V[midx2 + 14];
						m16 += matrix_U[midx14 + 3] * matrix_V[midx2 + 15];

					}

					int midx3 = (oc * Input_n * tile_h * tile_w * 4) + (n * tile_h * tile_w * 4) + (col / 2 + row_idxo3) * 4;
					...
					matrix_M[midx3] = m1 + m5 + m9 + sub_y1 + sub_y2;
					matrix_M[midx3 + 1] = sub_y1 - sub_y2 - m4 - m8 - m12;
					matrix_M[midx3 + 2] = m5 - m9 - m13 + sub_y3 + sub_y4;
					matrix_M[midx3 + 3] = sub_y3 - sub_y4 - m8 + m12 + m16;
...
void Mergematrix2(vector<float>& matrixOrigin, vector<float>& matrix11, vector<float>& matrix12, vector<float>& matrix21, vector<float>& matrix22, int Rows, int Cols)
{
	for (int i = 0; i < Rows; i++)
	{
		...
		for (int j = 0; j < Cols; j++)
		{
			...
			matrixOrigin[gidx] = matrix11[gidx];										//좌 상단행렬
			matrixOrigin[gidx + Rows * Cols] = matrix12[gidx];								//우 상단행렬 ////
			matrixOrigin[Rows * Cols * 2 + gidx] = matrix21[gidx];         			//좌 하단행렬
			matrixOrigin[Rows * Cols * 3 + gidx] = matrix22[gidx];				//우 하단행렬 ////
		}
	}
}

// 쉬트라센 알고리즘 함수
void Strassen(vector<float>& matrixU, vector<float>& matrixV, vector<float>& matrixM, int U_Row, int U_Col, int V_Row, int V_Col, int tile_h, int tile_w)
{

	if (1 == (U_Row / 4) % 2 || 1 == (U_Col / 4) % 2 || 1 == (V_Row / 4) % 2 || 1 == (V_Col / (4 * tile_h * tile_w)) % 2)
	{
		MatrixMulElement4X4(matrixU, matrixV, matrixM, U_Row, U_Col, V_Col, tile_h, tile_w);
		return;
	}
	else {
			...
			vector<float>  m1(newU_Row * newV_Col / 4), m2(newU_Row / 2 * newV_Col / 2), m3(newU_Row / 2 * newV_Col / 2), m4(newU_Row / 2 * newV_Col / 2), m5(newU_Row / 2 * newV_Col / 2), m6(newU_Row / 2 * newV_Col / 2), m7(newU_Row / 2 * newV_Col / 2);
			...
			Strassen(tempA, tempB, m1, newU_Row, newU_Col, newV_Row, newV_Col, tile_h, tile_w);    // m1=(a11+a11)(b11+b22)

			MatrixSum(a21, a22, tempA, newU_Row, newU_Col);            // a21+a22
			Strassen(tempA, b11, m2, newU_Row, newU_Col, newV_Row, newV_Col, tile_h, tile_w);      // m2=(a21+a22)b11

			MatrixSub(b12, b22, tempB, newV_Row, newV_Col);            // b12-b22
			Strassen(a11, tempB, m3, newU_Row, newU_Col, newV_Row, newV_Col, tile_h, tile_w);      // m3=a11(b12-b22)

			MatrixSub(b21, b11, tempB, newV_Row, newV_Col);            // b21-b11
			Strassen(a22, tempB, m4, newU_Row, newU_Col, newV_Row, newV_Col, tile_h, tile_w);      // m4=a22(b21-11)

			MatrixSum(a11, a12, tempA, newU_Row, newU_Col);            //  a11+a12
			Strassen(tempA, b22, m5, newU_Row, newU_Col, newV_Row, newV_Col, tile_h, tile_w); 	   // m5=(a11+a12)b22

			MatrixSub(a21, a11, tempA, newU_Row, newU_Col);            // a21-a11
			MatrixSum(b11, b12, tempB, newV_Row, newV_Col);            // b11+b12
			Strassen(tempA, tempB, m6, newU_Row, newU_Col, newV_Row, newV_Col, tile_h, tile_w);    // m6=(a21-a11)(b11+b12)

			MatrixSub(a12, a22, tempA, newU_Row, newU_Col);            // a12-a22
			MatrixSum(b21, b22, tempB, newV_Row, newV_Col);            // b21+b22
			Strassen(tempA, tempB, m7, newU_Row, newU_Col, newV_Row, newV_Col, tile_h, tile_w);    // m7 = (a12 - a22)(a12 - a22)
			...
			//재 병합
			Mergematrix2(matrixM, c11, c12, c21, c22, newU_Row / 2, newV_Col / 2);
		
	}
}

void mTransform(vector<float>& M, vector<float>& Y, int input_n, int output_c, int output_H, int output_W, int tile_n) {


	int temp_y = output_c * output_H * output_W;
	int temp_m = input_n * tile_n * 4;

	for (int n = 0; n < input_n; n++) // N
	{
		int temp_y2 = n * temp_y;
		int temp_m3 = n * tile_n * 4;

		for (int och = 0; och < output_c; och++) // OC
		{
			int temp_y3 = och * output_H * output_W + temp_y2;
			int temp_m2 = och * temp_m + temp_m3;

			for (int row = 0; row < output_H; row += 2)  // T , OH
			{
				int temp_y4 = row * output_W + temp_y3;
				int temp_y4_2 = (row + 1) * output_W + temp_y3;
				int tilerowc = (output_W / 2) * (row / 2);

				for (int col = 0; col < output_W; col += 2) // OW
				{
					int temp_y5 = col + temp_y4;
					int temp_y5_2 = col + temp_y4_2;

					int temp_mf = temp_m2 + (tilerowc + col / 2) * 4;

					Y[temp_y5] = M[temp_mf];
					Y[temp_y5 + 1] = M[temp_mf + 1];
					Y[temp_y5_2] = M[temp_mf + 2];
					Y[temp_y5_2 + 1] = M[temp_mf + 3];
...
int main()
{
	cout << "Strassen-Winograd Convolutions Test! \n\n";
	...
	int area_o = output_H * output_W;
	int tile_n = area_o / 4;
	int tile_h = output_H / 2;
	int tile_w = output_W / 2;
	vector<float> V(input_n * input_c * tile_n * 16);

	cout << "===== Strassen - Winograd Convolution ===== \n";
	...
	//valueCheck(U, output_c, input_c, 4, 4);
	// inputTransform 수행
	inputTransform5(V, input, input_n, input_c, input_H, input_W);
	//valueCheck(V, input_c, input_n, tile_n, 16);
	vector<float> M(input_n * output_c * output_H * output_W); // [OC][N][T][4]												   
	// Strassen - Winograd 수행
	Strassen(U, V, M, output_c * 4, input_c * 4, input_c * 4, input_n * 4 * tile_h * tile_w, tile_h, tile_w);

	// M[OC][N][T][4] - -> Y [N][OC][OH][OW]
	vector<float> Y(input_n * output_c * output_H * output_W); //  [N][OC][OH][OW]	
	mTransform(M, Y, input_n, output_c ,output_H, output_W, tile_n);
	...
	//valueCheck(M, output_c, input_n, tile_n, 4);
	cout << "======================================================" << endl;
	valueCheck(Y, input_n, output_c, output_H, output_W);
	...
	valueCheck(output, input_n, output_c, output_h_z, output_w_z);
	...
	cout << "======================================================" << endl;
	cout << frame_sec3 << "u sec (Convolution)" << endl;
	*/
