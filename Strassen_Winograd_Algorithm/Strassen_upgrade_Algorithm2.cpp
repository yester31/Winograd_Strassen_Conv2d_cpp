...
void MatrixMulElement(vector<int> Prev_matrix, vector<int> Post_matrix, vector<int>& Y_output, int prev_rows, int prev_colsAndPost_rows, int post_cols)
{
	...
	for (int i = 0; i < prev_rows; ++i) {
		...
		for (int j = 0; j < post_cols; ++j)
		{
			...
			{
				sum = Prev_matrix[temp1 + k] * Post_matrix[k * post_cols + j]
					+ Prev_matrix[temp1 + k + 4] * Post_matrix[(k + 4) * post_cols + j]
					+ Prev_matrix[temp1 + k + 8] * Post_matrix[(k + 8) * post_cols + j]
					+ Prev_matrix[temp1 + k + 12] * Post_matrix[(k + 12) * post_cols + j];
			}
			Y_output[temp2 + j] = (int)sum;
		}
	}
