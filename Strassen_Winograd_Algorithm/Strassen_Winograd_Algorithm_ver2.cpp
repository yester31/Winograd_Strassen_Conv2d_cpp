...
void valueCheckg(vector<float>& valueCheckInput, int input_n, int input_c, int input_h, int input_w, int offset = 0) {
	if (offset == 1) { input_n = 1; }
	...
	for (int ⁠n_idx = 0; ⁠n_idx < input_n; ⁠n_idx++)
	{
		...
		//int temp1 = input_w * input_h * input_c;
		for (int ⁠n_idx = 0; ⁠n_idx < input_n / jump; ⁠n_idx++)
		{
			//int temp2 = ⁠n_idx * temp1;
			for (int ⁠c_idx = 0; ⁠c_idx < input_c ; ⁠c_idx++)
			{
				...
				for (int ⁠h_idx = 0; ⁠h_idx < input_h; ⁠h_idx++)
				{
					...
					for (int w_idx = 0; w_idx < input_w; w_idx++)
					{
						int g_idx = w_idx + ⁠h_idx * input_w + ⁠c_idx * input_w * input_h + ⁠n_idx * input_c* input_w * input_h;
						cout << setw(5) << valueCheckInput[g_idx] << " ";
				...
				//int temp3 = ⁠c_idx * input_w * input_h + temp2;
				for (int ⁠h_idx2 = 0; ⁠h_idx2 < input_h; ⁠h_idx2++)
				{
					//int temp4 = ⁠h_idx * input_w + temp3;
					for (int w_idx2 = 0; w_idx2 < input_w; w_idx2++)
					{
						int g_idx2 = w_idx2 + ⁠h_idx2 * input_w + ⁠c_idx * input_w * input_h + ⁠n_idx * input_c * input_w * input_h + input_c * input_n * input_w * input_h / jump;
						cout << setw(5) << valueCheckInput[g_idx2] << " ";
...
void valueCheckgg(vector<float>& valueCheckInput, int input_n, int input_c, int input_h, int input_w, int offset = 0) {
	if (offset == 1) { input_n = 1; }
			...
			for (int ⁠c_idx = 0; ⁠c_idx < input_c; ⁠c_idx++)
			{
				...
				for (int s = 0; s < jump; s++) {
					...
					for (int ⁠h_idx = 0; ⁠h_idx < input_h; ⁠h_idx++)
					{
						...
						for (int w_idx = 0; w_idx < input_w; w_idx++)
						{
							int g_idx = w_idx + ⁠h_idx * input_w + ⁠c_idx * input_w * input_h + ⁠n_idx * input_c * input_w * input_h + input_c * input_n * input_w * input_h/jump * s;
							cout << setw(5) << valueCheckInput[g_idx] << " ";
...
void valueChecks(vector<float>& valueCheckInput, int input_n, int input_c, int input_h, int input_w, int offset = 0) {
	if (offset == 1) { input_n = 1; }
	...
	for (int ⁠n_idx = 0; ⁠n_idx < input_n; ⁠n_idx++)
	{
		...
		for (int ⁠c_idx = 0; ⁠c_idx < input_c; ⁠c_idx++)
		{
			...
			for (int ⁠h_idx = 0; ⁠h_idx < input_h; ⁠h_idx++)
			{
				...
				for (int w_idx = 0; w_idx < input_w; w_idx++)
				{
					int g_idx = w_idx + ⁠h_idx * input_w + ⁠c_idx * input_n * input_w * input_h + ⁠n_idx * input_w * input_h;
...
void valueChecks2(vector<float>& valueCheckInput, int input_n, int input_c, int input_h, int input_w, int offset = 0) {
	if (offset == 1) { input_n = 1; }


	int temp1 = input_n * input_w * input_h;
	for (int ⁠n_idx = 0; ⁠n_idx < input_n; ⁠n_idx++)
	{
		int temp2 = ⁠n_idx * 4;
		for (int ⁠c_idx = 0; ⁠c_idx < input_c; ⁠c_idx++)
		{
			int temp3 =⁠c_idx * temp1;
			for (int ⁠h_idx = 0; ⁠h_idx < input_h; ⁠h_idx++)
			{
				int temp4 = ⁠h_idx * input_w;
				for (int w_idx = 0; w_idx < input_w; w_idx++)
				{
					int g_idx = w_idx + temp4 + temp3 + temp2;
...
void MatrixMulElement4X4nn(vector<float> matrix_U, vector<float> matrix_V, vector<float>& matrix_M, int U_rows, int UcolsVrows, int V_cols, int tile_n)
{
	int Output_c = U_rows / 4;
	int Input_c = UcolsVrows / 4;
	int Input_n = V_cols / (4 * tile_n);
	//int tile_n = tile_n; // tile 총 개수 
	int Output_W = (int)sqrt(tile_n) * 2;
	int temp_o = Output_W * Output_W;

	for (int oc = 0; oc < Output_c; ++oc) { // OC

		int tempo1 = oc * Input_n * Output_W * Output_W;
		...
		for (int n = 0; n < Input_n; ++n) // N
		{
			...
			for (int row = 0; row < Output_W; row += 2) //TILE
			{
				...
				for (int col = 0; col < Output_W; col += 2)//TILE
				{
					...
					//int ot_idx1 = oc * Input_n * Output_W * Output_W  + n * Output_W * Output_W + row * Output_W + col;
...
void Mergematrix2(vector<float>& matrixOrigin, vector<float>& matrix11, vector<float>& matrix12, vector<float>& matrix21, vector<float>& matrix22, int Rows, int Cols)
{
	for (int i = 0; i < Rows; i++)
	{
		...
		for (int j = 0; j < Cols; j++)
		{
			...
			matrixOrigin[gidx + Cols + Cols] = matrix21[gidx2];								//우 상단행렬 ////

			matrixOrigin[gidx + Cols + Cols + Cols] = matrix22[gidx2];						//우 상단행렬 ////
...
void Mergematrix3(vector<float>& matrixOrigin, vector<float>& matrix11, vector<float>& matrix12, vector<float>& matrix21, vector<float>& matrix22, int Rows, int Cols)
{
	for (int i = 0; i < Rows; i++)
	{
		int temp = i * (Cols );
		...
		for (int j = 0; j < Cols; j++)
		{
			...
			matrixOrigin[Rows * Cols + gidx] = matrix12[gidx2];								//우 상단행렬 ////
			matrixOrigin[Rows * Cols * 2 + gidx] = matrix21[gidx2];								//우 상단행렬 ////
			matrixOrigin[Rows * Cols * 3 + gidx] = matrix22[gidx2];								//우 상단행렬 ////
...
void Mergematrix4(vector<float>& matrixOrigin, vector<float>& matrix11, vector<float>& matrix12, vector<float>& matrix21, vector<float>& matrix22, int Rows, int Cols)
{
	for (int i = 0; i < Rows; i++)
	{
		int temp = i * (Cols * 4);
		...
		for (int j = 0; j < Cols/4; j++)
		{
			...
			for (int k = 0; k < 4; k++) 
			{
			matrixOrigin[gidx + k] = matrix11[gidx2 + k];										//좌 상단행렬								
			matrixOrigin[gidx + 4 + k] = matrix12[gidx2 + k];         							//좌 하단행렬
			matrixOrigin[gidx + Cols + Cols + k] = matrix21[gidx2 + k];								//우 상단행렬 ////
			matrixOrigin[gidx + Cols + Cols + 4 + k] = matrix22[gidx2 + k];				//우 하단행렬 ////
			}
...
int main()
{
	...
	valueCheckgg(M, input_n, output_c, output_H, output_W);
